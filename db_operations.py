...
def add_sample_data():
    ...
    with current_app.app_context():
        if not Story.query.first():  # 如果数据库为空才添加示例数据
            ...
            # 创建选择
            choice1 = Choice(chapter_id=chapter1.id, text="进入丛林探险", next_chapter_id=chapter2.id)
            choice2 = Choice(chapter_id=chapter1.id, text="留在海滩等待救援", next_chapter_id=chapter3.id)
